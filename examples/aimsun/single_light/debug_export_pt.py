...
net_file = str(rep_seed) + '_net_delay.csv'
    ...
    if time % 900 == 0:
        ...
        for node_id in target_nodes:
            ...
            if node_id == 3329:
                ...
                gutil.extend((0,0,0,0,0))

            gutil.append(time_consumed[node_id])
            gutil.append(occurence[node_id])
            gutil.insert(0, node_id)
            gutil.insert(1, rep_seed)
            gutil.insert(2, time)

            with open('AM_util_debug.csv', 'a') as csvFile:
                csvwriter = csv.writer(csvFile)
                csvwriter.writerows([gutil])    
...
def AAPIPostManage(time, timeSta, timeTrans, acycle):
    ...
    if time % 900 == 0 and time != 0:
        estad = aapi.AKIEstGetParcialStatisticsSystem(timeSta, 0)
        if (estad.report==0):
            netdelay = estad.DTa
            print(rep_name, rep_seed)
            netl = [time, netdelay]
            with open(net_file, 'a') as my_csv:
                cwriter = csv.writer(my_csv)
                cwriter.writerows([netl,])

        for node_id in target_nodes:
            action_list = []
            gutil = sum(gUtil_at_interval(node_id, time_consumed, occurence, timeSta))
            util_list = [gutil]

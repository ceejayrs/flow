...
ADDITIONAL_ENV_PARAMS = {'target_nodes': [3370],
...
class SingleLightEnv(Env):
    ...
    def action_space(self):
        ...
        return Tuple(3 * (Discrete(86,),)) #5 (probabilities)
    ...
    def _apply_rl_actions(self, rl_actions):
        ...
        prob_phase = np.array([[actions[0], actions[1]]]) / 113 # [[p1],[p2]]
        # gives the length of barriers
        actionf = []  

        ### probability
        for i in range(len(prob_phase)): # [[1],[3]]
            ring = prob_phase[i] # [1]
            for j in range(len(ring)):
                new_phase1, new_phase2 = round(ring[j]*sum_barrier[0]), sum_barrier[0] - round(ring[j]*sum_barrier[0])
        ...
        self.sum_barrier = [sum(new_actions[0:2]), new_actions[2]]
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        print(f'{self.k.simulation.time:.0f}','\t', f'{reward:.4f}', '\t',
              ...
              sum(self.current_phase_timings[3:])+self.sum_interphase, self.sum_barrier)
